...
from utils.checks.role_checks import jr_mod_check, helper_check
...
async def verify(ctx: tanjun.abc.Context, ign: Annotated[tanjun.annotations.Str, "Your IGN"],
    ...
    roles = [role for role in list(ctx.member.role_ids) if
             role not in config['verify']['guild_member_roles'] and role != config['verify']['member_role_id']]

    await ctx.member.edit(roles=roles, reason='Verification Process')
    ...
    try:
        await ctx.member.edit(nickname=player["displayname"])
